...
pub fn normalize(input: &str) -> String {
    ...
    s = s.chars()
        .map(|c| {
            if c.is_alphanumeric() || c.is_whitespace() || c == '_' || c == '-' || c == '.' || c == '/' {
                c
            } else {
                ' '
            }
...
mod tests {
    ...
    fn strip_timestamp_prefix() {
        assert_eq!(normalize("[10:32] decided to use BTreeMap"), "decided to use btreemap");
    }

    #[test]
    fn preserve_file_paths() {
        assert_eq!(normalize("src/auth.rs needs a rewrite"), "src/auth.rs needs a rewrite");
    }
